...
[![ISA](https://img.shields.io/badge/ISA-116%20Opcodes-blueviolet)](docs/ESPEC.md)
...
1. A fixed 116-opcode ISA v1.16 (`0x00–0x49`, `0x4A–0x4D`, `0x50–0x53,0x56–0x57`, `0x60–0x69`, `0x6A–0x79`, `0x7A–0x7E`, `0xFF`): `TENSOR, ATTN, STREAM, FORK, ABORT, SENSE` + `NORM, FFN` (transformer) + `EMBED, ADD, SAMPLE` (thinking loop) + `COMPARE, JUMP, IF_EQUAL, IF_INTERRUPT` (control flow) + `MATVEC, MUL, SILU` (GEMV blocks) + `SSM_SCAN, SSM_RESET` (Mamba) + `CODEC_ENC, CODEC_DEC, AUDIO_ALIGN` (Mimi full-duplex) + `CTX_SWITCH, ROPE` (hybrid control) + `REMOTE_SPAWN, SIGNAL, SEND_TENSOR, BARRIER` (local cluster — RFC-0018) + `CONV, GATHER, SPIKE_STEP, DENOISE_STEP, FOREST, DISTANCE, RANK1_UPDATE, ODE_STEP` (universal waves — RFC-0004/0012–0015/0017) + `KV_TRUNCATE` + `SLICE` (RFC-0019) + `SORT, TOPK, ARGMAX, REDUCE, BROADCAST, PAD, TILE, TRANSPOSE` (shape — RFC-0027) + `SOFTMAX, GELU, SIGMOID, TANH, RELU, EXP, LOG, CLIP` (activations — RFC-0028) + `KV_COMPRESS, FLASH_ATTN, ATTN_SPARSE` (KV/attention — RFC-0029) + `STREAM_MERGE, VAD_DETECT, AUDIO_RESAMPLE, AUDIO_FILTER, AUDIO_WINDOW` (audio DSP — RFC-0031) + `DEPFORMER` (depformer step — RFC-0032) + `ARENA_ALLOC/RESET, MEMCPY/MEMSET` (memory core — RFC-0023) + `SNAPSHOT/RESTORE, PREFETCH/RESHAPE/CONCAT` (views & versions — RFC-0024) + `CAST, QUANTIZE, DEQUANT` (conversion — RFC-0025) + `RNG_*, HASH, CHECKSUM, HMAC` (determinism — RFC-0005) + telemetry/scheduler block (RFC-0006) + `LOADI, MOV` (RFC-0007) + `ADD_IMM, SUB_IMM, STEPS` (control-plane ALU — RFC-0026) + `CALL, RET` (subroutines — RFC-0034) + `RAG_INDEX_ADD/DEL, RAG_SEARCH, EMBED_LOOKUP, PQ_ENCODE/DECODE` (retrieval — RFC-0038) + `LOAD_MODEL, SPAWN_CONTEXT, KILL_CONTEXT, SET_MODEL` (system local — RFC-0039) (`INSTR_SIZE=32`, spec `docs/ESPEC.md`, RFCs `docs/RFC-*.md`). The assembler is 2-pass with labels (`LOOP:`, `JUMP LOOP`, `FORK Rd, LABEL`) and strict unknown-token rejection (RFC-0008).
