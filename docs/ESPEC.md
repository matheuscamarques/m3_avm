...
Date:     2026-09-10
Version:  ISA v1.12 (103 opcodes: 0x00-0x49 except 0x44, + 0x60-0x69,0x6A-0x79,0x7A-0x7C + 0xFF implemented; cumulative since v1.11)
...
`0x44`, `0x4A-0x79` and `0x7D-0xFE` are free. Allocated and IMPL nearby:
...
MODE=SCATTER_ADD`, `LINEAR_REG` -> `MATVEC+ADD`, `LOGISTIC_REG` ->
`MATVEC+ADD+SIGMOID`, `NEAREST_CENTROID` -> `DISTANCE TOPK=1`,
`MOE_DISPATCH` -> `MATVEC`+`SAMPLE-TOPK`+`GATHER`, `VQ_ENCODE` ->
`SLICE`+`DISTANCE-TOPK`, `STANDARDIZE` -> `REDUCE`+`MUL`/`ADD`
(Newton), `POWER_ITER` -> `MATVEC`+`MUL`/`ADD` (Newton),
`RBF_KERNEL` -> `DISTANCE`+`MUL`+`EXP` (all RFC-0030, never encoded).
Domain map: (1) dense matrix `MATVEC/MUL/CONV`;
...
- `ISA v1.12` = v1.11 + audio DSP family `0x45-0x49` (RFC-0031:
  `STREAM_MERGE`, `VAD_DETECT`, `AUDIO_RESAMPLE`, `AUDIO_FILTER`,
